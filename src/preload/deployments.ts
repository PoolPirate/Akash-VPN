...
// @ts-ignore
import type { AkashWallet } from '/wallet';
...
export const fetchDeploymentList = async (
  ...
  const deploymentCount = await fetchDeploymentCount(
    { owner: filters.owner },
    rpcNode
  );
...
export const watchLeaseLogs = async (
  address: string,
  provider: any,
  lease: any,
  message: any
  ...
  const socket = new WebSocket(`${proxyWSS}/${upstream}/${url}?follow=true`, [
    'log-protocol',
  ]);
...
export async function createLease(wallet: AkashWallet, bidId: BidID) {
  const [account] = wallet.accounts;
  const signer = wallet.offlineSigner;
  const { rpcNode } = getRpcNode();
