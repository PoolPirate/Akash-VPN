...
export const createAndBroadcastCertificate = async (
  rpcEndpoint: string,
  wallet: any
) => {
...
export const fetchCertificates = async (
  ...
  for (const cert of response.certificates) {
    if (
      ...
      cert.certificate.pubkey = Uint8Array.from(
        Object.values(cert.certificate.pubkey as object)
      );
    }
  }
...
export const loadCertificates = (walletId?: string) => {
  ...
  return certs.filter(
    (cert) => walletId === undefined || cert.walletId === walletId
  );
};
...
export const saveActiveSerial = (walletId: string, idx: number) => {
  localStorage.setItem(
    `active-certificate-serial-${walletId}`,
    JSON.stringify(idx)
  );
};
